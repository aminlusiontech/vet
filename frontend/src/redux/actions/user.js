...
export const loadUser = () => async (dispatch) => {
  try {
    ...
    // This prevents showing "Please login" errors when user is already authenticated
    // but the API call fails for other reasons (network, etc.)
    ...
    if (!isAuthError) {
      ...
      // For auth errors, just set loading to false without error
      dispatch({
        type: "LoadUserFail",
        payload: null, // Don't set error for auth failures
      });
    }
  }
};
...
export const updateUserInformation =
  (name, email, phoneNumber, ukaraNumber, password) => async (dispatch) => {
    try {
      dispatch({
...
export const updatUserAddress =
  (country, city, address1, address2, postCode, addressType) =>
  async (dispatch) => {
    try {
      dispatch({
