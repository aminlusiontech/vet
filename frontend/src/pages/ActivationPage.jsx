...
const ActivationPage = () => {
    ...
    // Fix double slash in URL (handle old email links)
    useEffect(() => {
    ...
    useEffect(() => {
        if (activation_token) {
            const verifyToken = async () => {
                try {
                    setLoading(true);
                    // Decode token to get user data (without creating account yet)
                    const response = await axios.post(`${server}/user/verify-activation-token`, {
                        activation_token
                    });
